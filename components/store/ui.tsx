...
export function PageIntro({ eyebrow, title, description, children }: {
    ...
    description?: string;
    children?: ReactNode;
}) { return <div className="page-intro" data-reveal><div><div className="breadcrumb"><Link href="/">Início</Link><span>/</span>{eyebrow ?? title}</div><h1>{title}</h1>{description && <p>{description}</p>}</div>{children}</div>; }
...
export function ProductCard({ product }: {
    product: Product;
}) { const { favorites, favorite } = useStore(); const href = product.kind === 'photo' ? (product.slug === 'polaroid' || product.slug === 'polaroid-com-ima' ? '/polaroid?produto=' + product.slug : product.slug === 'foto-3x4' ? '/foto-3x4' : '/revelacao?produto=' + product.slug) : product.kind === 'document' ? '/documentos' : '/produto/' + product.slug; return <article className="product-card" data-reveal><div className="product-image"><Link href={href} aria-label={product.name}>{product.image ? <PhotoImage src={product.image} alt={product.name + ' — imagem ilustrativa'} loading="lazy" width={600} height={650} sizes="(max-width: 720px) 45vw, (max-width: 1200px) 40vw, 25vw"/> : <div className="product-symbol"><Camera size={48} strokeWidth={1}/><span>Feito para você</span></div>}</Link>{product.kind === 'photo' && <span className="product-tag">Suas fotos, do seu jeito</span>}<button className={'heart-button ' + (favorites.includes(product.id) ? 'is-favorite' : '')} onClick={() => void favorite(product.id)} aria-pressed={favorites.includes(product.id)} aria-label={(favorites.includes(product.id) ? 'Desfavoritar ' : 'Favoritar ') + product.name}><Heart size={19} fill={favorites.includes(product.id) ? 'currentColor' : 'none'}/></button></div><div className="product-meta"><span>{product.category}</span><Link href={href}><h3>{product.name}</h3></Link><div className="product-bottom"><span>{product.price === null ? 'Consulte as opções' : <>{product.salePrice !== null && <del>{money(product.price)}</del>}{money(product.salePrice ?? product.price)}</>}</span><Link href={href} aria-label={'Ver ' + product.name}><ArrowRight size={20}/></Link></div></div></article>; }
