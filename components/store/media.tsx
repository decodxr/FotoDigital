...
export function PhotoImage({ src, alt = '', className = '', sizes, ...props }: ImgHTMLAttributes<HTMLImageElement>) { const [failed, setFailed] = useState(false); if (failed || !src)
...
} return <img {...props} src={src} alt={alt} className={className} srcSet={srcSet} sizes={sizes ?? (srcSet ? '(max-width: 640px) 92vw, (max-width: 1024px) 45vw, 45vw' : undefined)} decoding="async" onError={() => setFailed(true)}/>; }
