...
export async function signedS3Url(method: string, key: string, config: (s: string) => string, now = new Date(), headers: Record<string, string> = {}) {
    ...
    if (bucket.includes('/') || !key || key.split('/').some(part => !part || part === '.' || part === '..'))
        throw Error('Caminho inválido no Storage.');
    const stamp = now.toISOString().replace(/[:-]|\.\d{3}/g, ''), date = stamp.slice(0, 8), scope = `${date}/${region}/s3/aws4_request`;
    // Supabase's endpoint includes /storage/v1/s3. Preserve that prefix.
    url.pathname = url.pathname.replace(/\/$/, '') + '/' + [bucket, ...key.split('/')].map(encode).join('/');
    const signedHeaders: Record<string, string> = { ...headers, host: url.host };
    const names = Object.keys(signedHeaders).sort();
    const canonicalHeaders = names.map(name => name + ':' + signedHeaders[name].trim() + '\n').join('');
    const params: Record<string, string> = { 'X-Amz-Algorithm': 'AWS4-HMAC-SHA256', 'X-Amz-Credential': access + '/' + scope, 'X-Amz-Date': stamp, 'X-Amz-Expires': '300', 'X-Amz-SignedHeaders': names.join(';') };
    const qs = Object.entries(params).sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => encode(k) + '=' + encode(v)).join('&');
    const canonical = [method, url.pathname, qs, canonicalHeaders, names.join(';'), 'UNSIGNED-PAYLOAD'].join('\n');
