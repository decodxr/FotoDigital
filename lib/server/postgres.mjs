...
export function createPostgres(connection, options = {}) {
    ...
    return postgres(url.toString(), {
        ...
        fetch_types: false,
        max: options.max ?? 1,
        // Supavisor can drop replies when multiple implicit transactions are
        // pipelined on one socket. Postgres.js counts waiting queries separately
        // from the active one: 0 (not 1) disables that wire-level pipelining.
        // Promise.all remains supported; the driver queues each query safely.
        max_pipeline: 0,
        idle_timeout: 20,
        connect_timeout: 10,
        max_lifetime: 300,
